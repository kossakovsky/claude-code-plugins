...
import { coloredBar, label, getContextColor, BRIGHT_BLUE, RESET } from "../colors.js";
...
export function renderIdentityLine(ctx, alignLabels = false) {
    ...
    // Context text uses DeepSeek blue (94) — same as "DeepSeek 余额" in v1.1.3
    const contextLabelDisplay = `${BRIGHT_BLUE}${t("label.context")}${RESET}`;
    const contextValueDisplay = `${getContextColor(percent, colors, contextThresholds)}${contextValue}${RESET}`;
    let line = display?.showContextBar !== false
        ? `${contextLabelDisplay} ${coloredBar(percent, getAdaptiveBarWidth(), colors, contextThresholds)} ${contextValueDisplay}`
        : `${contextLabelDisplay} ${contextValueDisplay}`;
