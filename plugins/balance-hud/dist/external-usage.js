...
function sanitizeBalanceLabel(value) {
    ...
    // Balance labels from the auto_refresh daemon contain ANSI SGR color codes
    // matching v1.1.3 hud_balance.mjs. The daemon is a trusted local process.
    // Only strip OSC hyperlink sequences and C1 control chars — keep CSI/SGR colors.
    const sanitized = value
        .replace(/\x1B\][^\x07\x1B]*(?:\x07|\x1B\\)/g, '') // OSC sequences
        .replace(/\x1B[@-Z\\-_]/g, '') // 7-bit C1 / ESC Fe (non-CSI)
        .trim();
    if (!sanitized) {
        return null;
    }
...
export function getUsageFromExternalSnapshot(config, now = Date.now()) {
    ...
    try {
        ...
        // Structured balance consumed fields (from auto_refresh daemon)
        if (typeof parsed.balance_consumed === 'number' && Number.isFinite(parsed.balance_consumed)) {
            usage.balanceConsumed = parsed.balance_consumed;
        }
        if (typeof parsed.balance_pct === 'number' && Number.isFinite(parsed.balance_pct)) {
            usage.balancePct = parsed.balance_pct;
        }
        if (typeof parsed.balance_time === 'string' && parsed.balance_time.trim()) {
            usage.balanceTime = parsed.balance_time.trim();
        }
        if (parsed.balance_is_low === true) {
            usage.balanceIsLow = true;
        }
        return usage;
    }
    catch (err) {
